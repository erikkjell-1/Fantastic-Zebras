...
function App() {
  ...
  const [station, setStation] = useState<Data | null>(null)  // Data är en lista med "stoplocation"
  ...
  return (
    ...
    {station ?
      ...
      <p>
      From: {departure && departure.Departure.map(dep => (
        <p> {dep.time} {dep.name}  mot {dep.direction} </p>
      ))}
